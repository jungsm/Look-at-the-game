import Aos from './Aos'; // 상대 경로를 정확하게 수정해야 합니다.


export default [
  { 
    id: 1,
    tag: Aos,
    image: "https://cdn1.epicgames.com/offer/24b9b5e323bc40eea252a10cdd3b2f10/EGS_LeagueofLegends_RiotGames_S1_2560x1440-872a966297484acd0efe49f34edd5aed",
    title: '리그오브 레전드',
    content: '거리를 설정하면 가장 가까운 파이터를 소개해드려요! 서로 싸워보세요',
    date: 'April 24, 2019',
    number: 2
  },
  { 
    id: 2,
    tag: Aos,
    image: "https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota2_social.jpg",
    title: '도타2',
    content: '거리를 설정하면 가장 가까운 파이터를 소개해드려요! 서로 싸워보세요',
    date: 'April 24, 2019',
    number: 2
  },
  { 
    id: 3,
    tag: Aos,
    image: "https://static.heroesofthestorm.com/images/global/fb-share-1fcc54becc.jpg",
    title: '히어로즈 오브 더 스톰',
    content: '거리를 설정하면 가장 가까운 파이터를 소개해드려요! 서로 싸워보세요',
    date: 'April 24, 2019',
    number: 2
  },
  { 
    id: 4,
    tag: Aos,
    image: "https://i.namu.wiki/i/phJ9bfJSIbO20ApAiNXuQJ09pOCDQ8ZK227vY3Tq6DXuPgWLelFpc-Rfb89268nvI0jitg-y5fcG2z_xVrAA8A.webp",
    title: '하이퍼 유니버스',
    content: '거리를 설정하면 가장 가까운 파이터를 소개해드려요! 서로 싸워보세요',
    date: 'April 24, 2019',
    number: 2
  },
  { 
    id: 5,
    tag: Aos,
    image: "https://cdn.gamemeca.com/data_center/234/221/20210819190130.jpg",
    title: '카오스 온라인',
    content: '거리를 설정하면 가장 가까운 파이터를 소개해드려요! 서로 싸워보세요',
    date: 'April 24, 2019',
    number: 2
  }
];
